  ...
  <div class="mypage">
    ...
    <div class="mypage-container">
      <div class="messagebox">
        ...
        <form action="/user/messagesendsearch" method="post">
          <select id="messagesendsearchsel" name="messagesearchsel"> 
            <option value="finduserid">아이디</option>
            <option value="findcontent">내용</option>
          </select>
          <input type="text" placeholder="쪽지 검색" name="searchtext">
          <input type="hidden"value="${principal.user.id}" id="searchuser"name="searchuser">
          <button id="messagesendsearch-btn">검색</button>
        </form>
        <button><a href="/user/sendmessage">전체보기</a></button>
        </div>
        <div class="message-list">
          <li>
            <div class="message-list-header"></div>
            <div class="message-list-header">받는 사람</div>
          ...
          <c:forEach var="message" items="${messages.content}">
          	<li>
            	<div></div>
            	<div>${message.receiveusernick }</div>
            	<div><a href="/user/sendmessagedetail/${message.id }" class="messagecontent">${message.contents}</a></div>
