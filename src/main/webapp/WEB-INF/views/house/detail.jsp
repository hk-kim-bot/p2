  ...
  <div class="container" id="wrrap">
    ...
    <div class="content1">
      <div class="sub-title">
        ...
        <div class="img">
        	<c:choose>
        	 	<c:when test="${house.users.profileimage eq null }">
					<img src="/img/user-profile.png" alt="프로필사진" class="img-fluid">
        	 	</c:when>
        	 	<c:otherwise>
		          <img src="${house.users.profileimage}" alt="프로필사진" class="img-fluid">
        	 	</c:otherwise>
        	</c:choose>
        	<div class="host-info-message">
        		<p><span>${house.users.nickname}님에게 메시지 보내기</span></p>
        		<input type="hidden" value="${house.users.id }" id="hiddenuserid">
        		<textarea id="message-contents" rows="5"cols="25"  style="resize: none;"></textarea>
        		<c:choose>
        			<c:when test="${principal eq null }">
        				<button id="require-login">보내기</button><button id="message-close">닫기</button>
        			</c:when>
        			<c:otherwise>
        				<input type="hidden" value="${principal.user.id }" id="hiddensendid">
		        		<button id="message-btn">보내기</button><button id="message-close">닫기</button>
        			</c:otherwise>
        		</c:choose>
        	</div>
        </div>
...
<%@ include file="../layout/footer.jsp" %>
  ...
  <script>
  	let profileimg = document.querySelector(".img-fluid");
  	let messagecontents = document.querySelector(".host-info-message");
  	let messageclose =document.querySelector("#message-close");
  	profileimg.addEventListener("click",()=>{
  		messagecontents.style.display="block";
  	})
  	messageclose.addEventListener("click",()=>{
  		messagecontents.style.display="none";
  	})
  </script>
