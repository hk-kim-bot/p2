...
import com.cos.sharehouse.dto.Naver_Sens_V2;
...
// 			session.setAttribute("SPRING_SECURITY_CONTEXT", securityContext);
	...
	@PostMapping("/api/pwdmailCheck")
	public ResponseDto<Users> pwdmailCheck(@RequestParam(value="userid")String userid,@RequestParam(value="email")String email2){
		Users user = userservice.finduseridmail(userid,email2);
		return new ResponseDto<Users>(HttpStatus.OK.value(),user);
	}
	@PostMapping("/auth/changepwdProc")
	public ResponseDto<Integer> changepwdProc(@RequestBody Users user){
		userservice.비밀번호변경(user);
		return new ResponseDto<Integer>(HttpStatus.OK.value(),1);
	}
	//sms 문자 전송 맵핑작성
	@PostMapping("/auth/sendmessage")
	public ResponseDto<String> sendRandomMessage(@RequestParam(value="tel") String tel) {
...
//		    message.send_msg(tel, numStr);
		    ...
		    return new ResponseDto<String>(HttpStatus.OK.value(), numStr);
	}
	@PostMapping("/auth/roleupdate/{id}")
	public ResponseDto<Integer> authupdate(@PathVariable int id){
		userservice.권한설정(id);
		
		return new ResponseDto<Integer>(HttpStatus.OK.value(),1);
	}
	@DeleteMapping("/auth/liketounlike/{id}")
	public ResponseDto<Integer> liketounlike(@PathVariable String id,@AuthenticationPrincipal PrincipalDetail principal){
		productService.unlike(id,Integer.toString( principal.getUser().getId()));
		
		return new ResponseDto<Integer>(HttpStatus.OK.value(),1);
	}
	@PostMapping("/auth/unliketolike/{id}")
	public ResponseDto<Integer> unliketolike(@PathVariable String id,@AuthenticationPrincipal PrincipalDetail principal){
		productService.tolike(id,Integer.toString( principal.getUser().getId()));
