...
//		Page<House> searchtype(@Param("type")String[] type,Pageable page);//이건 성공
		...
		@Query(value="select * from House h where h.type in :type and  h.category in :category and h.bedroom in :bedroom and h.amenities like %:amenities%"
				,countQuery="select count(*) from House h where h.type in :type and  h.category in :category and h.bedroom in :bedroom and h.amenities like %:amenities%",nativeQuery=true)
...
//		Page<House> 찜보기(Pageable page,@Param("userid")int userid);
		@Query(value="select *from House h where h.type on :type and h.category in :category and h.bedroom >= 4 and hamenities like  %:amenities%",
				countQuery="select count(*) from House h where h.type on :type and h.category in :category and h.bedroom >= 4 and hamenities like  %:amenities%",nativeQuery=true)
		Page<House> searchhouseover4(@Param("type")String[] type,@Param("category")String[] category,@Param("amenities") String amenities,Pageable page);
